...
const Layout: React.FC<Props> = ({ children }) => {
  ...
  //         // @ts-ignore
  //         uids: isVendor
  //           ? chatList?.map((chat) => chat.agent)
  //           : chatList?.map((chat) => chat.vendor),
  //       }
  //     : null,
  // });
  ...
  const filteredChats = chats.filter(chat => chat.userDetails.name.toLowerCase().includes(searchQuery.trim().toLowerCase()) || chat.lastMessage.toLowerCase().includes(searchQuery.trim().toLowerCase()))
